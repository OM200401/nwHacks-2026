...
const currentCommit = {
  ...
  description: "Added logging for admin token bypass and improved JWT validation",
  author: "Sarah Chen",
  ...
  branch: "main",
  files: [
    { name: "OldSessionHandler.js", additions: 12, deletions: 5, path: "src/legacy", active: true },
    { name: "AuthProvider.tsx", additions: 45, deletions: 23, path: "src/auth", active: false },
    { name: "useAuth.ts", additions: 8, deletions: 2, path: "src/hooks", active: false },
  ]
};
...
const relevantSections: CodeSection[] = [
  {
    ...
    lines: [
      ...
      { type: "context", line: 35, content: "    // HACK: Skip validation for admin tokens", relevant: true },
      { type: "deletion", line: 36, content: "    if (token.startsWith('admin_')) return true;", relevant: true },
      { type: "addition", line: 36, content: "    if (token.startsWith('admin_')) {", relevant: true },
      { type: "addition", line: 37, content: "      console.warn('Admin token bypass used');", relevant: true },
      { type: "addition", line: 38, content: "      return true;" },
      { type: "addition", line: 39, content: "    }" },
      { type: "context", line: 40, content: "" },
      { type: "context", line: 41, content: "    const parts = token.split('.');" },
      { type: "deletion", line: 42, content: "    return parts.length >= 2;", relevant: true },
      { type: "addition", line: 42, content: "    // Should be 3 parts for JWT but legacy tokens have 2" },
      { type: "addition", line: 43, content: "    return parts.length >= 2 && parts.length <= 3;", relevant: true },
  ...
  },
  {
    ...
    lines: [
      ...
      { type: "context", line: 46, content: "    const [user, setUser] = useState(null);" },
      { type: "addition", line: 47, content: "    const [isValidating, setIsValidating] = useState(true);", relevant: true },
      { type: "context", line: 48, content: "" },
      { type: "context", line: 49, content: "    useEffect(() => {" },
      { type: "deletion", line: 50, content: "      const session = sessionHandler.getSession();", relevant: true },
      { type: "addition", line: 50, content: "      const session = await sessionHandler.getSession();", relevant: true },
      { type: "addition", line: 51, content: "      setIsValidating(false);", relevant: true },
      { type: "context", line: 52, content: "      if (session) setUser(session.user);" },
  ...
  },
  {
    ...
    lines: [
      { type: "context", line: 22, content: "  async function refreshToken() {" },
      { type: "addition", line: 23, content: "    console.log('Refreshing auth token...');", relevant: true },
      { type: "context", line: 24, content: "    try {" },
      { type: "context", line: 25, content: "      const newToken = await api.refresh();" },
      { type: "addition", line: 26, content: "      console.log('Token refreshed successfully');", relevant: true },
      { type: "context", line: 27, content: "      return newToken;" },
      { type: "context", line: 28, content: "    } catch (e) {" },
      { type: "addition", line: 29, content: "      console.error('Token refresh failed:', e);", relevant: true },
    ...
    <div className="h-screen bg-background flex flex-col overflow-hidden">
      ...
      <header className="border-b border-border bg-gradient-to-r from-primary/5 to-transparent flex-shrink-0">
        <div className="px-4 py-3">
          ...
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 border border-primary/20">
                ...
                <code className="text-sm font-mono font-bold text-primary">{activeCommit?.hash || currentCommit.hash}</code>
              </div>
              <span className="text-xs text-muted-foreground font-mono">on {currentCommit.branch}</span>
            </div>
          ...
          {/* Commit message */}
          <h1 className="text-lg font-semibold text-foreground mb-1">{activeCommit?.summary || currentCommit.message}</h1>
          <p className="text-sm text-muted-foreground mb-3">{activeCommit?.description || currentCommit.description}</p>
          ...
          <div className="flex items-center gap-4 text-xs">
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 rounded-full bg-primary/20 flex items-center justify-center text-primary font-medium text-[10px]">
                {(activeCommit?.author || currentCommit.author).split(' ').map(n => n[0]).join('')}
              </div>
              <span className="text-foreground font-medium">{activeCommit?.author || currentCommit.author}</span>
            </div>
            ...
            <div className="flex items-center gap-2 ml-auto">
              <span className="flex items-center gap-1 text-muted-foreground">
                ...
                {activeCommit?.fileCount || currentCommit.files.length} files changed
              </span>
            </div>
          </div>
        </div>
                ...
                {commitNodes.map((node) => (
                  ...
                  >
                    ...
                    <div className={cn(
                      ...
                      node.relevance === "high" && "border-green-500 bg-green-500/10",
                      node.relevance === "medium" && "border-yellow-500 bg-yellow-500/10",
                      node.relevance === "low" && "border-muted-foreground/50 bg-muted/50",
                    ...
                    <div className="flex-1 min-w-0 pt-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className={cn(
                          ...
                          node.relevance === "high" && "bg-green-500/20 text-green-600",
                          node.relevance === "medium" && "bg-yellow-500/20 text-yellow-600",
                          node.relevance === "low" && "bg-muted text-muted-foreground"
                        )}>
                          {node.relevance}
                        </span>
                      </div>
                      ...
                      <div className={cn(
                        ...
                        <span className="flex items-center gap-1">
                          <User className={selectedCommitNode ? "w-3 h-3" : "w-4 h-4"} />
                          {selectedCommitNode ? node.author.split(' ')[0] : node.author}
                        </span>
                        <span>•</span>
                        <span className="flex items-center gap-1">
                          <Clock className={selectedCommitNode ? "w-3 h-3" : "w-4 h-4"} />
                          {node.date}
                        </span>
                        <span>•</span>
                        <span className="flex items-center gap-1">
                          <FileCode className={selectedCommitNode ? "w-3 h-3" : "w-4 h-4"} />
          ...
          {selectedCommitNode && (
            <>
              ...
              <div className="flex items-center gap-2 px-4 py-2 border-b border-border bg-primary/5">
                ...
                <span className="text-sm font-medium text-foreground">Code Details</span>
                      ...
                      >
                        <div className="flex items-center gap-2 min-w-0">
                          <div className={cn(
                            ...
                            section.relevance === "low" && "bg-muted-foreground"
                          )} />
                          <div className="min-w-0">
                            <div className="flex items-center gap-2 text-sm">
                              <FileCode className="w-4 h-4 text-primary flex-shrink-0" />
                              <span className="font-medium text-foreground truncate">{section.file}</span>
                              <code className="text-xs font-mono text-primary truncate">{section.functionName}</code>
                            </div>
                          </div>
                        </div>
                            ...
                            {section.lines.map((line, i) => (
                              <div
                                ...
                                className={cn(
                                  ...
                                  line.relevant && "ring-1 ring-inset ring-primary/40 bg-primary/5"
                                )}
                              >
                                {line.relevant && (
